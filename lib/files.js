...
function isInRepository(repositoryPath, filename, callback) {
    ...
    git.Repository.open(repositoryPath).then(function (repo) {
        ...
        commit.getEntry(filename).then(function () {
            callback(true);
        }, function () {
            callback(false);
        });
...
function latexDependencies(filename, callback) {
    fs.readFile(filename, function (err, data) {
        ...
        async.map(dependencies, function (dependency, callback) {
            fs.stat(dependency, function (err) {
                if (err) {
                    fs.stat(dependency + '.tex', function (err) {
...
var identifyAssociatedFiles = module.exports.identifyAssociatedFiles = function (texFilenames, callback) {
    async.map(texFilenames, identifyFilesAssociatedWithFile, function (err, results) {
        if (err) {
            callback(err, []);
        } else {
