...
var HOSTNAME = 'ximera.stevens.edu';
var PORT = 443;
                    ...
                    gitCommit(repositoryPath, sha, function (err, commit) {
                        if (err) {
                            ...
                            var payload = {
                                ...
                                parents: commit.parents().map(function (s) {
                                    return s.toString();
                                })
                            };
                            ...
                            restful('PUT', url, JSON.stringify(payload), function (err) {
                                return callback(err, sha, publicationLocation);
                            });
                        }
                    });
                } else {
...
var publishFile = function (kind, repositoryPath, filename, callback) {
    headCommitSha(repositoryPath, function (err, sha) {
        if (err) {
            ...
            // HTML files should be extensionless
